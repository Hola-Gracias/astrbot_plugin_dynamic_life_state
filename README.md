...
- **业务周期**：`generate_time` 同时定义每日生成时间和业务周期边界。例如 `07:00` 下，业务日期 `2026-06-30` 对应 `[2026-06-30 07:00, 2026-07-01 07:00)`；任务实际执行时间不会改变周期
- **首次对话懒生成**：当前业务周期状态不存在时自动触发生成
- **当前时段匹配**：根据带时区的自然日期时间从 `timeline` 中选中对应时段（支持自然关键词如"上午""下午"和具体区间如 `08:00-09:00`、`23:00-01:00`）
  ...
  - 时段与业务周期取交集；被周期边界切开的同名时段复用同一条安排
...
| `/dls full <YYYY-MM-DD>` | 查看指定日期的完整状态 |
...
| `generate_time` | string | 每日生成时间和业务周期边界 (HH:MM) |
...
业务日期不是自然日。所有自然时段和具体时间区间都会先展开为带日期的实际区间，再与当前业务周期取交集。例如 `generate_time=07:00` 时：

- 业务日期 `2026-06-30` 的“凌晨”为 `2026-07-01 00:00–06:00`。
- “早上”会被切为 `2026-06-30 07:00–09:00` 和 `2026-07-01 06:00–07:00`，两段使用同一项安排。
